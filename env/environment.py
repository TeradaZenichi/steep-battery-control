...
class Simulation:
    def __init__(self, config=None, dataframe=None):
        ...
        if "timestamp" in self.dataframe:
            self.dataframe["timestamp"] = pd.to_datetime(self.dataframe["timestamp"], format="%d/%m/%Y %H:%M", dayfirst=True, errors="coerce")
    ...
    def record_state(self, raw_state, masked_state=None, unnormalized_state=None):
        snap = {"step": self.current_step, "timestamp": self.current_datetime, "raw_state": np.array(raw_state, copy=True)}
        if masked_state is not None: snap["masked_state"] = np.array(masked_state, copy=True)
        if unnormalized_state is not None: snap["raw_state_unscaled"] = np.array(unnormalized_state, copy=True)
    ...
    def record_action(self, pbess, pev, xpvc, pgrid, ppv_used, ppv_curtailed, pbess_cmd=None, pev_cmd=None):
        d = {
            "step": self.current_step, "timestamp": self.current_datetime, "timestep_minutes": self.timestep,
            "PBESS": float(pbess), "PEV": float(pev), "XPV": float(xpvc), "pgrid": float(pgrid),
            "ppv_used": float(ppv_used), "ppv_curtailed": float(ppv_curtailed),
        }
        if pbess_cmd is not None: d["PBESS_cmd"] = float(pbess_cmd)
        if pev_cmd is not None: d["Pev_cmd"] = float(pev_cmd)
        self.action_history.append(d)

    def _pad(self, seq, n):
        if n is None: return list(seq)
        if not seq: return []
        if n <= len(seq): return list(seq[-n:])
        return [seq[0]] * (n - len(seq)) + list(seq)

    def get_action_history(self, last_n=None): return self._pad(self.action_history, last_n)
    def get_state_history(self, last_n=None): return self._pad(self.state_history, last_n)
    ...
    def get_unnormalized_state_history(self, last_n=None):
        ...
        return [{"step": s["step"], "timestamp": s["timestamp"], "raw_state_unscaled": np.array(s["raw_state_unscaled"], copy=True)} for s in unscaled]
    ...
    def set_start_date(self, start_date):
        ...
        self.set_start_step(int(np.searchsorted(self.timestamps.to_numpy(dtype="datetime64[ns]"), target, side="left")))
...
class BESS:
    ...
    def _pmax_charge(self, soc):
        return min(self.Pmax, max(0.0, float(np.interp(float(np.clip(soc, self.soc_min, self.soc_max)), self._soc_knots, self._pmax_charge_curve))))
    def _pmax_discharge(self, soc):
        return min(self.Pmax, max(0.0, float(np.interp(float(np.clip(soc, self.soc_min, self.soc_max)), self._soc_knots, self._pmax_discharge_curve))))
    ...
    def step(self, action):
        ...
        e_leak = float(np.clip(self.β, 0.0, 1.0)) * e_prev
        e_next = float(np.clip(e_leak + self.sim.dt * (max(p_cmd, 0.0) * self.η - max(-p_cmd, 0.0) / self.η), self.soc_min * self.Emax, self.soc_max * self.Emax))
        ...
        thr = abs(d_ctrl)
        self.soh = max(0.0, self.soh - (1.0 - self.β) * thr / (self.Emax * self.DoD))
        self._costdeg = thr * self._deg_cost_per_kwh
        self._penalty = 0.0
        return self._pbess, self._costdeg, self._penalty, {
            "p_cmd": p_cmd, "p_used": self._pbess, "pbess_charge_kw": self._pbess_c, "pbess_discharge_kw": self._pbess_d,
            "soc": self.soc, "soh": self.soh, "throughput_kWh": thr, "leakage_kWh": max(0.0, e_prev - e_leak),
        }

    def get_obs(self): return np.array([self.soc, self.soh], np.float32)
...
class EV:
    def __init__(self, config=None, sim=None):
        ...
        self.soc_column = config.get("soc_column", config.get("column", "ev_status"))
        usable = (self.Emax * self.DoD) or 1.0
        self._deg_cost_per_kwh = self.capex / (self.ncycles * usable)
    ...
    def reset(self, soc_init=None):
        if soc_init is not None: self.soc0 = soc_init
        self.soc, self.soh, self.connected = self.soc0, self.soh0, False
        self._pev = self._pev_c = self._pev_d = self._costdeg = self._penalty = 0.0
        self._pending_absence_reset = False

    def step(self, action):
        status = self.sim.get_value(self.status_column, 0.0)
        prev = self.connected
        ...
        if self.connected and not prev:
            self.soc = float(np.clip(status, self.soc_min, self.soc_max))
            self.soh = self.soh0

        if not self.connected:
            if prev:
                self._penalty = max(0.0, 1.0 - self.soc) * self.Emax * (self.penalty_departure or 1.0)
                self._pending_absence_reset = True
            else:
                self.soc = 0.0
            return 0.0, 0.0, self._penalty, {"event": "departure" if prev else "absent", "status_signal": status, "connected": False, "p_cmd": 0.0, "p_used": 0.0, "pev_charge_kw": 0.0, "pev_discharge_kw": 0.0, "soc": self.soc, "soh": self.soh}

        p_cmd = float(np.clip(action, -self.Pmax_d, self.Pmax_c))
        e_prev = self.soc * self.Emax
        e_next = float(np.clip(e_prev + self.sim.dt * (max(p_cmd, 0.0) * self.η - max(-p_cmd, 0.0) / self.η), self.soc_min * self.Emax, self.soc_max * self.Emax))
        d = e_next - e_prev
        self.soc = e_next / self.Emax
        ...
        self.soh = max(0.0, self.soh - (1.0 - self.β) * thr / (self.Emax * self.DoD))
        self._costdeg = thr * self._deg_cost_per_kwh
        return self._pev, self._costdeg, 0.0, {"event": "arrival" if (self.connected and not prev) else "connected", "status_signal": status, "connected": True, "p_cmd": p_cmd, "p_used": self._pev, "pev_charge_kw": self._pev_c, "pev_discharge_kw": self._pev_d, "soc": self.soc, "soh": self.soh}

    def get_obs(self): return np.array([self.soc, self.soh, 1.0 if self.connected else 0.0], np.float32)
...
class Grid:
    ...
    def reset(self): self._pgrid = self._cost = self._revenue = self._penalty = self.tariff = 0.0
    ...
    def step(self, action):
        ...
        return self._cost, self._revenue, self._penalty, {"p_cmd": p, "p_used": p, "tariff": self.tariff, "energy_kWh": e, "import_cost": self._cost, "export_revenue": self._revenue, "penalty_power": ex}

    def get_obs(self): return np.array([self._pgrid, self.tariff], np.float32)
...
class PV:
    ...
    def reset(self): self._ppv = self._curt = self._available_kw = 0.0
    def _get_available_kw(self): return self.sim.get_value(self.column, 0.0) / 1000.0
    ...
    def step(self, action):
        ...
        return self._ppv, {"pv_available_kw": self._available_kw, "pv_used_kw": self._ppv, "pv_curtailed_kw": self._available_kw - self._ppv, "curtailment": self._curt}

    def get_obs(self): return np.array([self._get_available_kw() / (self.Pmax or 1.0)], np.float32)
...
class Load:
    ...
    def reset(self): self._pload = 0.0
    def _get_demand_kw(self): return self.sim.get_value(self.column, 0.0) / 1000.0
    def step(self): self._pload = self._get_demand_kw(); return self._pload
    def get_obs(self): return np.array([self._pload], np.float32)
...
class SmartHomeEnv(gym.Env):
    def __init__(self, config=None, dataframe=None, render_mode=None, days=None, state_mask=None, start_date=None):
        ...
        if days is not None: g["simulation_days"] = days
        self.sim = Simulation(g, dataframe=dataframe)
        if start_date is not None: self.sim.set_start_date(pd.to_datetime(start_date, dayfirst=True))
        ...
        self._feature_names = (
            ...
            ["month_sin","month_cos","day_sin","day_cos","hour_sin","hour_cos","minute_sin","minute_cos","weekday_sin","weekday_cos"] +
            ["bess_soc","bess_soh"] + ["ev_soc","ev_soh","ev_connected"] + ["ev_shortfall"] +
            ["pv_available"] + ["load","tariff","net_load"]
        )
        self.state_mask = np.array(state_mask, bool) if state_mask is not None else None
        self.state_feature_labels = [n for n, k in zip(self._feature_names, self.state_mask)] if self.state_mask is not None else list(self._feature_names)
    ...
    def reset(self, seed=None, options=None):
        ...
        self.sim.reset(); self.bess.reset(); self.grid.reset(); self.load.reset(); self.pv.reset(); self.ev.reset()
        self.done = False
    ...
    def step(self, action):
        ...
        self.sim.record_action(self.bess._pbess, self.ev._pev, self.pv._curt, pgrid, self.pv._ppv, ppv_curtailed, pbess_cmd=bi.get("p_cmd"), pev_cmd=ei.get("p_cmd"))
        self.grid.step(pgrid)

        obs = self._get_obs()
        pv_curt_cost = ppv_curtailed * self.sim.dt * self.pv_curtailment_penalty
        r = -(self.bess._costdeg + self.bess._penalty + self.ev._costdeg + self.ev._penalty + self.grid._cost - self.grid._revenue + self.grid._penalty + pv_curt_cost)
    ...
    def _build_time_features(self, ts):
        if pd.isna(ts): return [0.0] * 10
        def cyc(v, p): a = 2 * np.pi * (v / p); return [float(np.sin(a)), float(np.cos(a))]
    ...
    def _get_obs(self):
        ...
        load_kw = float(self.load.get_obs()[0])
        net_kw = load_kw - self.pv._ppv
        feats = climate + time + b + e + [shortfall] + [pv_av / (self.pv.Pmax or 1.0)] + [load_kw / pnorm, self.grid.tariff / (self.sim.tariff_base or 1.0), net_kw / pnorm]
        unscaled = climate_raw + time + b + e + [shortfall] + [pv_av] + [load_kw, self.grid.tariff, net_kw]
    ...
    def build_operation_dataframe(self):
        ...
        comp = unscaled["raw_state_unscaled"].apply(lambda a: (v := np.asarray(a, float), pd.Series({"soc_bess": v[idx_bess], "soc_ev": v[idx_ev], "ev_status": v[idx_ev_status], "load_kw": v[idx_load]}))[1])
        comb = act.join(comp, how="left")

        out = pd.DataFrame({
            "Pgrid": comb["pgrid"], "Ppv": comb["ppv_used"], "Load": comb["load_kw"],
            "PBESS": comb["PBESS"], "EBESS": comb["soc_bess"] * self.bess.Emax,
            "Pev": comb["PEV"], "Eev": comb["soc_ev"] * self.ev.Emax, "ev_status": comb["ev_status"],
        })

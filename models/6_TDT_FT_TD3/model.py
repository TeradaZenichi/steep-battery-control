...
class Critic(nn.Module):
    """Transformer encoder over states + MLP heads, mirroring the actor backbone."""
    ...
    def forward(self, state_seq: torch.Tensor, action: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        time_idx = torch.arange(seq_len, device=device, dtype=torch.long).unsqueeze(0).expand(bs, -1)
        state_embed = self.state_encoder(state_seq) + self.time_embedding(time_idx)
        encoded = self.transformer(state_embed)
        encoded = self.norm(encoded)
        pooled = encoded[:, -1, :]
        sa = torch.cat([pooled, action], dim=-1)

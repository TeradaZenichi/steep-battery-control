...
ATT_MEMv2_ROOT = Path(__file__).resolve().parents[1]  # .../models/ATT_MEMv2
ALGO_ROOT    = Path(__file__).resolve().parent      # .../models/ATT_MEMv2/2-RL
sys.path.insert(0, str(ALGO_ROOT))
sys.path.insert(1, str(ATT_MEMv2_ROOT))
...
class Train:
    def __init__(self, tariff: str):
        ...
        with open(ATT_MEMv2_ROOT / "model.json", encoding="utf-8") as f:
        ...
        self.folder = PROJECT_ROOT / "Results" / "train" / "ATT_MEMv2" / "2-RL" / self.tariff
        ...
        self.critic_cfg.setdefault("d_model", int(self.model_cfg["actor"].get("d_model", 128)))
        ...
        self.critic_cfg.setdefault("d_ff", int(self.model_cfg["actor"].get("d_ff", 512)))
        self.critic_cfg.setdefault("mem_hidden", int(self.model_cfg["actor"].get("mem_hidden", 128)))
        self.critic_cfg.setdefault("mem_layers", int(self.model_cfg["actor"].get("mem_layers", 1)))
    ...
    def _load_il_hpo(self, tariff: str) -> dict:
        il_path = PROJECT_ROOT / "Results" / "train" / "ATT_MEMv2" / "1-IL" / tariff / "best_params.json"

...
from model import ImitationLSTM  # type: ignore  # pylint: disable=wrong-import-position
...
def _build_sequences(features: np.ndarray, targets: np.ndarray, seq_len: int) -> Tuple[np.ndarray, np.ndarray]:
    total = len(features)
    if total < seq_len:
        return np.empty((0, seq_len, features.shape[1]), dtype=np.float32), np.empty((0, targets.shape[1]), dtype=np.float32)
    seq_feats = []
    seq_tgts = []
    for idx in range(seq_len - 1, total):
        seq_feats.append(features[idx - seq_len + 1 : idx + 1])
        seq_tgts.append(targets[idx])
    return np.stack(seq_feats, axis=0), np.stack(seq_tgts, axis=0)
...
def build_teacher_dataset(
    ...
    seq_feature_chunks: List[np.ndarray] = []
    seq_target_chunks: List[np.ndarray] = []
    ...
    for idx, scenario in enumerate(scenarios, start=1):
        ...
        seq_features, seq_targets = _build_sequences(features, target_block, seq_len)
        ...
        seq_feature_chunks.append(seq_features)
        seq_target_chunks.append(seq_targets)

    if not seq_feature_chunks:
        raise RuntimeError("No scenarios produced training data; try reducing seq_len or revisiting inputs.")

    features = np.concatenate(seq_feature_chunks, axis=0)
    targets = np.concatenate(seq_target_chunks, axis=0)
...
def train_model(
    ...
    features, targets, feature_labels, mask_vector, mask_repr = build_teacher_dataset(
        TRAIN_SCENARIOS,
        seq_len=seq_len,
...
def save_training_artifacts(hparams: HyperParameters, result: Dict[str, object]) -> None:
    ...
    mask_vector = np.asarray(result["mask_vector"], dtype=bool)
    torch.save(
        {
    ...
    ax.set_title("LSTM imitation training")
    ax.legend()

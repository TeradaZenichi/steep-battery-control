...
class ReplayBuffer:
    ...
    giving a ~2Ã—history_len reduction (e.g. ~384Ã— for history_len=192).
    """
    ...
    def _store_transition(self, obs: np.ndarray, act: np.ndarray, rew: float,
        ...
        # Accept sequences (history_len, obs_dim) â€“ take last frame only
        if obs.ndim == 2:
            obs = obs[-1]
        if next_obs.ndim == 2:
            next_obs = next_obs[-1]
    ...
    def __init__(self, config: dict):
        def _get_first(keys, default=None):
            ...
            if default is not None:
                return default
            raise KeyError(f"Missing required config keys: {keys}")

        self.seed       = config["seed"]
        self.days       = config["days"]
        self.gamma      = float(_get_first(["Î³", "γ"]))
        self.tau        = float(_get_first(["Ï„", "τ"]))
        self.batch_size     = config["batch_size"]
        self.update_steps   = config["update_steps"]
        self.actor_lr       = config["actor_lr"]
        self.critic_lr      = config["critic_lr"]
        self.alpha_lr       = float(_get_first(["Î±_lr", "α_lr"]))
...
class Temperature(torch.nn.Module):
    ...
    def loss(self, log_prob: torch.Tensor) -> torch.Tensor:
        # SAC original (minimizaÃ§Ã£o em log_alpha):
        # L = - log_alpha * (log_prob + target_entropy)
        # grad = -(log_prob + target_entropy); se entropia estÃ¡ abaixo da meta (log_prob menos negativo), grad > 0 -> alpha aumenta.

...
from model import ImitationLSTM  # type: ignore

CONFIG_PATH = Path("data/parameters.json")
RESULTS_BASE = Path("Results")
MODEL_SUBDIR = "5_LSTM_FT_TD3"
...
BASE_RUN_LABEL = "rl-finetune-lstm-td3"
DATASETS = ["WY", "CY"]
...
def actor_policy(actor: Actor, device: torch.device, seq_len: int) -> Callable:
    def _policy(env: SmartHomeEnv, obs_seq: np.ndarray) -> np.ndarray:
        obs_t = torch.as_tensor(obs_seq, dtype=torch.float32, device=device).unsqueeze(0)
        with torch.no_grad():
...
def teacher_policy(results_df: pd.DataFrame) -> Callable:
    def _policy(env: SmartHomeEnv, _obs_seq: np.ndarray) -> np.ndarray:
...
def rollout_env(
    ...
    while not env.done:
        ...
        window = np.concatenate([window[1:], obs[None, :]], axis=0)

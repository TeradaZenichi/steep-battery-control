...
try:
    ...
    from .model import Critic, ImitationLSTM
...
def expand_runs(cfg: dict) -> list[dict]:
    ...
    for idx, run in enumerate(runs):
        ...
        if not label:
            base_label = base.get("run_label", "rl-finetune-lstm-td3")
...
class Actor(nn.Module):
    def __init__(self, hparams: HyperParameters, action_low: np.ndarray, action_high: np.ndarray):
        super().__init__()
        self.core = ImitationLSTM(hparams)
...
def main() -> None:
    ...
    for tariff in tariffs:
        ...
        results_base = Path("RESULTS") / tariff_label / "5_LSTM_FT_TD3" / "train"
        run_dir = results_base

        checkpoint = Path(cfg.get("checkpoint")) if cfg.get("checkpoint") else Path("Results") / tariff_label / "2_LSTM_IL" / "best.pt"

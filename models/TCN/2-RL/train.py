...
class Train:
    def __init__(self, tariff: str):
        ...
        self.folder = PROJECT_ROOT / "Results" / "train" / "TCN" / "2-RL" / self.tariff
    ...
    def _eval_parallel(self, runs, actor_state_cpu, deterministic: bool, eval_desc: str, eval_workers: int) -> float:
        rewards = []
    ...
    def _eval_sequential(self, runs, actor_state_cpu, deterministic: bool, eval_desc: str) -> float:
        rewards = []
        with tqdm(total=len(runs), desc=f"{eval_desc} (seq)", position=2, dynamic_ncols=True, leave=False) as p_eval:
    ...
    def eval(self, deterministic: bool = True) -> float:
        ...
        if eval_workers <= 1:
            return self._eval_sequential(runs, actor_state_cpu, deterministic, eval_desc)

        try:
            return self._eval_parallel(runs, actor_state_cpu, deterministic, eval_desc, eval_workers)

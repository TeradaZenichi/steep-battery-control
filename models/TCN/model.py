...
class TCNActor(nn.Module):
    ...
    Architecture:
        Input (B, T, input_dim)
        → transpose to (B, input_dim, T)
        → TemporalConvNet (causal dilated convolutions)
        → last-timestep pooling → (B, hidden)
    ...
    def _dist_params(self, x: torch.Tensor):
        ...
        # x_seq: (B, T, input_dim) → transpose to (B, input_dim, T) for Conv1d

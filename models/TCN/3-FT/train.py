...
ALGO_ROOT    = Path(__file__).resolve().parent      # .../models/TCN/3-FT
sys.path.insert(0, str(ALGO_ROOT))
sys.path.insert(1, str(TCN_ROOT))
...
class Train:
    def __init__(self, tariff: str):
        ...
        self.env_cy = SmartHomeEnv(
            self.episodegen.df_cy, self.parameters,
            start=self.episodegen.sample("cy"), days=self.hp.days,
            BESS_SoC=0.5, tariff=self.tariff, track_operation=False,
        )
        self.env_wy = SmartHomeEnv(
            self.episodegen.df_wy, self.parameters,
            start=self.episodegen.sample("wy"), days=self.hp.days,
            BESS_SoC=0.5, tariff=self.tariff, track_operation=False,
        )
        ...
        self.folder = PROJECT_ROOT / "Results" / "train" / "TCN" / "3-FT" / self.tariff
        ...
        self.il_ckpt = PROJECT_ROOT / str(
            self.ft_cfg.get("il_checkpoint_pattern", "Results/train/TCN/1-IL/{tariff}/best.pth").format(
                tariff=self.tariff
            )
        )
        ...
        self.temperature = Temperature(init_log_alpha=0.0, target_entropy=self.hp.target_entropy).to(DEVICE)
        ...
        self.audit_df = pd.DataFrame(columns=[
            "episode", "train_reward_total",
            "eval_reward_det", "eval_reward", "eval_reward_ma", "eval_reward_stoch",
            "checkpoint_score", "checkpoint_metric",
            "best_train_reward", "best_eval_reward", "best_eval_ma", "best_checkpoint_score",
            "q1_mean", "q2_mean", "backup_mean", "backup_abs_max",
            "q_next_mean", "logp_mean", "logp_min",
            "n_updates", "steps", "phase", "buffer_size", "alpha",
            "lambda", "dual_enabled",
            "cost_mean", "cost_p95", "frac_violation",
            "critic_loss", "actor_loss",
            "actor_term_entropy", "actor_term_q", "actor_term_dual", "actor_term_il_reg",
            "alpha_loss", "no_improve_episodes", "no_improve_evals",
        ])
    ...
    def eval(self, deterministic: bool = True) -> float:
        ...
        with tqdm(total=len(runs), desc=eval_desc, position=2, dynamic_ncols=True, leave=False) as p_eval:
            with ProcessPoolExecutor(max_workers=eval_workers) as executor:
                futures = [
                    executor.submit(
                        _eval_worker, run, self.parameters, self.tariff,
                        self.actor_cfg, actor_state_cpu, self.episode_length,
                        self.history_len, deterministic,
                    )
                    for run in runs
                ]
                for idx, fut in enumerate(as_completed(futures), start=1):
                    ...
                    p_eval.set_postfix({"scenario": f"{idx}/{len(runs)}", "reward": f"{total_reward:.2f}", "avg": f"{running_mean:.2f}", "w": int(eval_workers)})
                    p_eval.update(1)
    ...
    def _aggregate_episode_update_metrics(self, q_start: int) -> dict:
        ...
        return {
            "n_updates": n_updates,
            "q1_mean": np.nan, "q2_mean": np.nan,
            "backup_mean": np.nan, "backup_abs_max": np.nan,
            "q_next_mean": np.nan, "logp_mean": np.nan, "logp_min": np.nan,
            "cost_mean_ep": np.nan, "cost_p95_ep": np.nan, "frac_violation_ep": np.nan,
            "critic_loss_ep": np.nan, "actor_loss_ep": np.nan,
            "actor_term_entropy_ep": np.nan, "actor_term_q_ep": np.nan,
            "actor_term_dual_ep": np.nan, "actor_term_il_reg_ep": np.nan,
    ...
    def _resolve_history_len(self, tariff: str) -> int:
        ...
        il_path = PROJECT_ROOT / "Results" / "train" / "TCN" / "1-IL" / tariff / "best_params.json"
    ...
    def _update_train_postfix(self, p_outer, train_total, eval_reward_det, eval_reward_ma, eval_reward_stoch, checkpoint_score, metrics, no_improve_episodes, no_improve_evals=0):
    ...
    def train(self):
        ...
        for episode in (p_outer := tqdm(range(self.hp.train_episodes), desc="Training Episodes", position=0, dynamic_ncols=True)):
            ...
            row = self._build_audit_row(
                episode=episode, train_total=train_total,
                eval_reward_det=eval_reward_det, eval_reward_ma=eval_reward_ma,
                eval_reward_stoch=eval_reward_stoch, checkpoint_score=checkpoint_score,
                metrics=metrics, steps=steps,
                no_improve_episodes=no_improve_episodes, no_improve_evals=no_improve_evals,
            )

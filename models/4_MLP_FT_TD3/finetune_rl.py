...
def make_grad_scaler(amp_enabled: bool, device: torch.device) -> GradScaler:
    ...
    except TypeError:  # older torch without device_type
...
def expand_runs(cfg: dict) -> list[dict]:
    ...
    for idx, run in enumerate(runs):
        ...
        if not label:
            base_label = base.get("run_label", "rl-finetune-mlp-td3")
...
def pretrain_critics(
    ...
    while step_counter < steps:
        ...
        obs, _ = env.reset()
        if buffer is None:
            buffer = ReplayBuffer(buffer_size, env.observation_space.shape[0], env.action_space.shape[0], device)

        if actor_target is None:
            actor_target = Actor(actor.core.hparams, env.action_space.low, env.action_space.high).to(device)
            critic_target = Critic(actor.core.hparams, env.action_space.shape[0], actor.core.hparams.hidden_sizes, activation="relu").to(device)
...
def build_env(
    ...
    with open(config_path, "r", encoding="utf-8") as fp:
        config = json.load(fp)
    if tariff_override:
        config["Grid"]["tariff_column"] = tariff_override
    import pandas as pd
...
def main() -> None:
    ...
    for tariff in tariffs:
        ...
        print(f"=== Tariff {tariff_label} ===")
        results_base = Path("RESULTS") / tariff_label / "4_MLP_FT_TD3" / "train"
        run_dir = results_base

        checkpoint = Path(cfg.get("checkpoint")) if cfg.get("checkpoint") else Path("Results") / tariff_label / "1_MLP_IL" / "best.pt"
        ...
        for idx, run_cfg in enumerate(runs, start=1):
            print(f"=== Run {idx}/{len(runs)}: {run_cfg['run_label']} ===")
            ...
            if actor is None:
                ...
                critic = Critic(hparams, action_dim, hparams.hidden_sizes, activation="relu").to(device)
            else:
            ...
            if idx == 1:
                ...
                if critic_ckpt:
                    critic_state = torch.load(Path(critic_ckpt), map_location=device)

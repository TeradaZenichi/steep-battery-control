...
ATTv2_ROOT   = Path(__file__).resolve().parents[1]  # .../models/ATTv2
ALGO_ROOT    = Path(__file__).resolve().parent      # .../models/ATTv2/2-RL
sys.path.insert(0, str(ALGO_ROOT))
sys.path.insert(1, str(ATTv2_ROOT))
...
class Train:
    def __init__(self, tariff: str):
        ...
        with open(ATTv2_ROOT / "model.json", encoding="utf-8") as f:
        ...
        self.folder = PROJECT_ROOT / "Results" / "train" / "ATTv2" / "2-RL" / self.tariff
    ...
    def _load_il_hpo(self, tariff: str) -> dict:
        il_path = PROJECT_ROOT / "Results" / "train" / "ATTv2" / "1-IL" / tariff / "best_params.json"

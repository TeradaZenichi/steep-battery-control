...
# CONFIG (EDIT HERE)
# =============================================================================
MODEL_NAME = Path(__file__).resolve().parent.name     # "3-FT"
SPLIT = "test"
...
def _read_csv(csv_path: Path) -> pd.DataFrame:
    ...
    df = pd.read_csv(csv_path, sep=",", parse_dates=["timestamp"], index_col="timestamp").sort_index()
...
def _slice_window(df: pd.DataFrame, start_date: str, days: int) -> pd.DataFrame:
    ...
    if w.empty:
        raise ValueError(f"No data in selected window.\nstart={start} end(exclusive)={end}\nCSV time range: {df.index.min()} .. {df.index.max()}")
    return w
...
def _plot_actor(df: pd.DataFrame, title: str, out_path: Path) -> None:
    ...
    colors = {"BESS": "#1f77b4", "EV": "#ff7f0e", "PV": "#2ca02c", "Load": "#d62728", "Grid": "#9467bd"}
    ...
    if PLOT_PV_AVAILABLE:
        ...
        ax0.plot(df.index, pv_available.values, label="PV_available", color=colors["PV"], linestyle="--")
    ...
    ax0.bar(df.index, df["PBESS"].astype(float).values, width=bar_width, color=colors["BESS"], alpha=0.7, label="PBESS")
    ax0.bar(df.index, df["PEV"].astype(float).values, width=bar_width, color=colors["EV"], alpha=0.7, label="PEV")
...
def main() -> None:
    ...
    for tariff in TARIFFS:
        out_dir = RESULTS_ROOT / "figures" / "ATT" / MODEL_NAME / tariff
        ...
        for entry in entries:
            ...
            actor_csv = RESULTS_ROOT / SPLIT / "ATT" / MODEL_NAME / tariff / f"{name}{ACTOR_SUFFIX}"
            teacher_csv = RESULTS_ROOT / SPLIT / "ATT" / MODEL_NAME / tariff / f"{name}{TEACHER_SUFFIX}"

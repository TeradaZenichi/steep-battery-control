...
TCNv2_ROOT   = Path(__file__).resolve().parents[1]  # .../models/TCNv2
ALGO_ROOT    = Path(__file__).resolve().parent      # .../models/TCNv2/2-RL
sys.path.insert(0, str(ALGO_ROOT))
sys.path.insert(1, str(TCNv2_ROOT))
...
class Train:
    def __init__(self, tariff: str):
        ...
        self.folder = PROJECT_ROOT / "Results" / "train" / "TCNv2" / "2-RL" / self.tariff
        ...
        self.critic_cfg.setdefault("num_channels", list(self.model_cfg["actor"].get("num_channels", [128, 128, 128])))
        self.critic_cfg.setdefault("kernel_size", int(self.model_cfg["actor"].get("kernel_size", 3)))
        self.critic_cfg.setdefault("dropout", float(self.model_cfg["actor"].get("dropout", 0.0)))
    ...
    def _load_il_hpo(self, tariff: str) -> dict:
        il_path = PROJECT_ROOT / "Results" / "train" / "TCNv2" / "1-IL" / tariff / "best_params.json"

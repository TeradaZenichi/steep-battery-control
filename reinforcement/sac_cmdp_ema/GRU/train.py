"""SAC-CMDP + EMA-backup training (arch inferred from directory name).

Identical to the sac_cmdp baseline EXCEPT the continuation action a' ~ π(s')
in the Bellman target (reward and cost critics) is sampled from a lagged EMA
copy of the policy instead of the live actor. This is the one drift-mitigation
lever that the v9 / cmdp / penalty / rlpd lines never tested.

Motivation (Results/Insights v9): the standard target re-evaluates good
transitions using the current policy's continuation; when the live policy
degrades after the early peak, those target values collapse with it, driving
the late critic drift. A lagged EMA continuation keeps the target anchored to
the better (peak-era) policy for longer.

Config flags (train):
    backup_via_ema : bool  — enable EMA continuation in the backup (default true)
    ema_backup_tau : float — Polyak factor for the backup EMA (default 0.995)

Everything else is imported from `reinforcement.sac_cmdp_ema.utils`, which
re-exports the shared sac_cmdp stack and only overrides the two critic-step
functions. The baseline `sac_cmdp` package is never modified.

Run (from project root):
    python reinforcement/sac_cmdp_ema/<ARCH>/train.py
"""
import importlib
import json
import os
...
from reinforcement.sac_cmdp_ema.utils import (
...
def train_tariff(tariff: str):
    ...
    backup_via_ema = bool(cfg["train"].get("backup_via_ema", True))
    ema_backup_tau = float(cfg["train"].get("ema_backup_tau", 0.995))
    ...
    # Dedicated lagged policy used ONLY for the Bellman-target continuation.
    ema_backup = EMA(actor, tau=ema_backup_tau)
    ...
    out_dir = PROJECT_ROOT / "paper" / "train" / "sac_cmdp_ema" / ARCH_NAME / f"{tariff}{RUN_SUFFIX}"
    ...
    backup_actor = ema_backup.model if backup_via_ema else None
    checkpoint_scores = {
        "best": -float("inf"),
    ...
    step_count = 0
    tqdm.write(
        f"[setup] CMDP+EMA-backup {tariff}: costs={list(costs_cfg)} "
        f"safe_rollout={safe_rollout} backup_via_ema={backup_via_ema} ema_backup_tau={ema_backup_tau}"
    )
    ...
    def snapshot():
        return {
            ...
            "ema_backup": {k: v.detach().cpu().clone() for k, v in ema_backup.state_dict().items()},
            ...
            with UpdateBar(n_updates, ep) as update_bar:
                for _ in range(n_updates):
                    ...
                    row = reward_critic_step(
                        ...
                        opt_reward, grad_clip=hp.grad_clip, backup_actor=backup_actor,
                    )
                    for name, ccfg in costs_cfg.items():

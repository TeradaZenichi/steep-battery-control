"""SAC-CMDP + RLPD training (arch is inferred from the directory name).

Adds RLPD-style prior data on top of the sac_cmdp baseline:
- a static `prior_buffer` populated once at startup from RB/RBS demonstration
  episodes saved under paper/demos/<source>/<tariff>/*.npz
- every SAC update samples 50% from the online buffer and 50% from the prior
  buffer; the batches are concatenated before the critic and actor steps
- critic and actor must already use LayerNorm (set in models/<arch>/model.json)

Everything else (CMDP cost critics, Lagrangian λ, EMA, eval pipeline, etc.) is
imported from `reinforcement.sac_cmdp.utils` unchanged. The only structural
differences vs the sac_cmdp baseline are the prior buffer setup and the
merged-batch sampling.

Run (from project root):
    python reinforcement/sac_cmdp_rlpd/<ARCH>/train.py
"""
import importlib
import json
import os
...
def train_tariff(tariff: str):
    ...
    # ---- RLPD prior buffer ---------------------------------------------------
    # Recompute costs at load time so the prior buffer matches the trainer's
    # current CMDP cost configuration. Without this, demos collected before a
    # cost config change would carry stale cost annotations.
    def _prior_cost_fn(obs_np, act_np):
        if not costs_cfg:
            return None
        obs_t = torch.as_tensor(obs_np, dtype=torch.float32, device=device).unsqueeze(0)
        act_t = torch.as_tensor(act_np, dtype=torch.float32, device=device).view(1, -1)
    ...
    prior_buffer = load_prior_buffer(
        ...
        sources=list(rlpd_cfg.get("sources", ["RB", "RBS"])),
        tariff=tariff,
        obs_dim=model_cfg["actor"]["input_dim"],
        act_dim=3,
        device=device,
        ...
        cost_names=list(costs_cfg),
        cost_compute_fn=_prior_cost_fn,
        capacity=rlpd_cfg.get("prior_capacity"),
    )
    prior_ratio = float(rlpd_cfg.get("prior_ratio", 0.5))
    if not (0.0 <= prior_ratio <= 1.0):
        raise ValueError(f"rlpd.prior_ratio must be in [0,1]; got {prior_ratio}")
    bs_prior = int(round(hp.batch_size * prior_ratio))
    bs_online = hp.batch_size - bs_prior
    tqdm.write(
        f"[rlpd] {tariff}: prior_buffer={prior_buffer.size} transitions, "
        f"batch split = online {bs_online} / prior {bs_prior}"
    )

    episodes = EpisodeGen(cfg, str(PROJECT_ROOT / "data"), seed=hp.seed)
    out_dir = PROJECT_ROOT / "paper" / "train" / "sac_cmdp_rlpd" / ARCH_NAME / f"{tariff}{RUN_SUFFIX}"
    ...
    step_count = 0
    tqdm.write(f"[setup] CMDP+RLPD {tariff}: costs={list(costs_cfg)} safe_rollout={safe_rollout}")
    ...
    def mixed_sample():
        """50/50 batch from online + prior buffers (per `prior_ratio`)."""
        if bs_online > 0 and buffer.size > 0:
            b_on = buffer.sample(bs_online)
        else:
            b_on = None
        if bs_prior > 0 and prior_buffer.size > 0:
            b_pr = prior_buffer.sample(bs_prior)
        else:
            b_pr = None
        if b_on is None:
            return b_pr
        ...
        if ep >= hp.warmup_episodes:
            ...
            with UpdateBar(n_updates, ep) as update_bar:
                for _ in range(n_updates):
                    ...
                    batch = mixed_sample()
                    if batch is None:
                        # Both buffers empty (should not happen after warmup).
                        continue

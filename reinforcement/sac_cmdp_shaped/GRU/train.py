"""GRU SAC-CMDP + anticipatory BESS-arbitrage shaping.

Identical to the sac_cmdp baseline (same utils, CMDP cost critics, eval), but
turns on a potential-based reward shaping term Φ_bess (weight
`shaping_bess_omega`) that rewards holding stored BESS energy when prices are
low — guiding PV-surplus arbitrage. Potential-based shaping is policy-invariant
(Ng-Harada-Russell), so the optimal policy is unchanged; only learning is guided.
Evaluation/test run on the bare env (raw reward), so metrics stay comparable.

Run:
    python reinforcement/sac_cmdp_shaped/GRU/train.py
"""
import json
import os
import sys
...
def train_tariff(tariff: str):
    ...
    out_dir = PROJECT_ROOT / "paper" / "train" / "sac_cmdp_shaped" / ARCH_NAME / f"{tariff}{RUN_SUFFIX}"
    ...
    for ep, pbar in episode_bars(hp.train_episodes, hp.warmup_episodes, tariff):
        ...
        envs = {
            stream: AnticipatoryShapingWrapper(
                ...
                bess_omega=cfg["train"].get("shaping_bess_omega", 0.0),
            )
            for stream in ("cy", "wy")
        }

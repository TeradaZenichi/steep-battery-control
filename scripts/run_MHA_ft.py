"""MHA fine-tuning pack. Run: python scripts/run_MHA_ft.py

Trains supervised IL per tariff (if missing) and then runs SAC fine-tuning
variants (penalty + CMDP) starting from the corresponding BC checkpoint.

MHA is the slower arch; comment out ablations in EXPERIMENTS if compute
is tight.
"""
import sys

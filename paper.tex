...
\title{Demonstration-Free Constrained Soft Actor-Critic with Potential-Based Shaping for Residential Energy Management under Dynamic Tariffs}
...
\begin{highlights}
    \item Demonstration-free SAC for home PV, battery, EV and grid control
    \item CMDP-Lagrangian safety separates economic reward from feasibility cost
    \item Potential-based BESS shaping stabilizes training without expert demos
    \item Dropout and randomized critic ensembles target long-horizon overestimation
    \item Policies are tested across five tariffs and three sequence encoders
...
Demonstration-free learning \sep Constrained Markov decision process \sep
Soft actor-critic \sep Potential-based reward shaping
\end{keyword}
...
Residential energy systems that combine photovoltaic generation, battery storage, electric vehicles, and grid interaction require real-time control under uncertainty, intermittent renewables, dynamic tariffs, and state-dependent operational limits. Reinforcement learning is attractive for this setting because it can adapt policies directly from closed-loop operation, but practical \gls{SHEMS} deployments cannot usually rely on expensive expert demonstrations from optimization-based teachers. This paper asks whether \gls{SAC} can be trained for home energy management in a stable, safe, and economically competitive way without behavior-cloning warm-starts. We study a common simulation environment with explicit action projection, five tariff schemes, three sequence encoders, and a unified \gls{SAC} training harness. The central result is that separating economics from feasibility through a \gls{CMDP}-Lagrangian formulation, and adding a simple potential-based \gls{BESS} shaping signal, makes from-scratch \gls{SAC} a viable demonstration-free alternative to behavior-cloning warm-starts. We further test critic-side anti-overestimation mechanisms, including dropout regularization and randomized critic ensembles, to reduce the residual drift observed under long-horizon discounting.
...
Furthermore, gaps persist in safety, architectural comparison, evaluation, and dependence on expert demonstrations. The study in~\cite{CEUSTERS2023101202} emphasizes the importance of safety layers and explicit constraints in multi-energy systems, yet many learning-based \gls{SHEMS} studies do not report a dedicated feasibility layer and instead rely solely on action clipping. This limitation is particularly relevant in systems with \gls{BESS} and \gls{EV}, where admissible actions depend on state of charge, availability, power limits, and degradation. Additionally, recent reviews on reinforcement learning for \gls{SHEMS} identify challenges related to safety, scalability, validation in real applications, and comparability across studies~\cite{en17246420}. These gaps motivate the present work, which evaluates demonstration-free \gls{SAC} variants with physics-constrained action projection, \gls{CMDP} dual feasibility regularization, potential-based reward shaping, anti-overestimation critics, and multiple neural architectures under a multi-tariff protocol for real-time residential operation with \gls{PV}, \gls{BESS}, \gls{EV}, and grid interaction. Behavior cloning from a \gls{MILP} teacher is retained as a comparison baseline rather than as a required training ingredient.
...
TW
& 5m & P,B,E,G & P,B,E(V2G) & SAC(P,C)+$\Phi$+FT+Reg. & Suite$^\dagger$ & Proj+Dual & Flat,ToU,Sol,Wnd,SW & C\$,Gap,Viol,Stat & H$^{\ddagger}$ \\
...
\textbf{Methods.} BC = behavior cloning (supervised learning from expert demonstrations); RL = reinforcement learning; DDPG = deep deterministic policy gradient; MADDPG = multi-agent DDPG; PPO = proximal policy optimization; SAC = soft actor--critic; TD3 = twin delayed DDPG; DQN = deep Q-network; MORL = multi-objective RL; RIQL = risk-informed Q-learning; LSAC = Lagrangian SAC; SAC(P,C) = penalty-based and CMDP-Lagrangian SAC variants; $\Phi$ = potential-based reward shaping; FT = behavior-cloning fine-tuning reference; Reg. = critic regularization/ensembles.
...
This paper studies safe \gls{SHEMS} operation with \gls{PV}, \gls{BESS}, \gls{EV}, and grid interaction, and asks whether reinforcement learning controllers can be trained reliably under realistic constraints and dynamic tariffs without expert demonstrations. Its central finding is that a from-scratch \gls{SAC} controller becomes stable and economically competitive when safety is handled as a \gls{CMDP} with separate cost critics and the training reward is augmented with a lightweight potential-based \gls{BESS} shaping signal. Behavior cloning from a \gls{MILP} teacher is therefore treated as a strong but expensive reference, not as a prerequisite. The main contributions are:

\begin{itemize}
    \item We characterize seed-dependent critic drift in from-scratch \gls{SAC} for residential energy control and show why single-seed conclusions can be misleading. Multi-seed training reveals that some apparent drift is seed noise, while potential-based shaping substantially reduces the variance of the final policy.

    \item We compare a penalty-based \gls{SAC} variant, where feasibility costs enter the scalar reward, against a \gls{CMDP}-Lagrangian formulation with separate cost critics and a learned dual variable, under an identical environment, hyperparameters, and evaluation protocol. This isolates the methodological value of separating economic reward from safety cost.

    \item We introduce a demonstration-free potential-based \gls{BESS} shaping term $\Phi_{\mathrm{BESS}}(s)$ that gives the agent a cheap mathematical hint about when stored energy is valuable. We test whether this shaping can play the practical stabilizing role normally sought through behavior-cloning warm-starts, while avoiding the need for an expert controller and demonstration dataset.

    \item We test critic-side anti-overestimation mechanisms for long-horizon \gls{SAC}: a DroQ-style dropout critic and a REDQ-style randomized critic ensemble. These ablations target the residual value overestimation that remains under high discount factors.

    \item All learned policies are evaluated through a safety projection enforcing state-dependent power limits of \gls{BESS}, \gls{EV}, and \gls{PV}, and are compared against rule-based heuristic baselines and a behavior-cloning fine-tuning reference in absolute operating cost, cost reduction, feasibility-violation rate, storage use, and robustness across five tariff structures and three neural encoders (\gls{GRU}, multi-head attention, \gls{TCN}).
\end{itemize}
...
% by X% on average" and "CMDP+PBRS reduces cost over RBS by Y% while matching
% or approaching the BC+FT comparator".
...
%   framework, including the SAC variants, CMDP safety, PBRS, critic
%   regularization, and the BC/FT comparator. Section 4 describes the
...
This section presents the proposed system for real-time residential energy management with distributed energy resources. The framework combines a simulation environment and learning-based controllers for sequential decision-making under different tariff signals. The environment models the interactions among residential demand, photovoltaic generation, a stationary \gls{BESS}, an \gls{EV}, and the distribution grid. The main controllers are trained by reinforcement learning directly in this environment; an \gls{MILP} formulation is additionally used to generate expert actions for the behavior-cloning and fine-tuning references. To preserve operational feasibility, the actor output is constrained by state-dependent device limits, while an additional safety projection is used to enforce grid exchange limits during validation and testing. The complete framework is evaluated under five tariff profiles: a flat tariff, a time-of-use \gls{ToU} tariff, and three renewable-dependent dynamic tariffs based on solar, wind, and combined solar-wind availability. Figure~\ref{fig:methodology} summarizes the main components of the proposed methodology and their interconnections. This section details the simulation environment (Section~\ref{subsec:environment}) and the formal control problem (Section~\ref{subsec:problem_formalization}); the learning framework and the experimental setup follow in Sections~\ref{sec:learning_framework} and~\ref{sec:experimental_setup}.
...
This section details the learning-based controllers. The main training path is demonstration-free: \gls{SAC} learns directly from simulator interaction under alternative safety formulations, shaping signals, and critic regularization mechanisms. A \gls{MILP} teacher (Section~\ref{subsec:milp_teacher}) and behavior-cloning policy (Section~\ref{subsec:bc}) are retained as reference baselines and as the source of the fine-tuning comparator, allowing the shaped from-scratch policies to be compared against a demonstration-based warm-start. The neural encoders and the feasibility-aware action layer shared by all controllers are described in Section~\ref{subsec:architectures}.

\subsection{Learning pipeline}
\label{subsec:pipeline}

The framework studies two families of controllers for the residential energy management problem. The first family is demonstration-free and trains \gls{SAC} directly in the simulator from a randomly initialized actor. Within this family we compare penalty-based safety, \gls{CMDP}-Lagrangian safety, potential-based \gls{BESS} shaping, and critic anti-overestimation variants. The second family is demonstration-based and uses a \gls{MILP} teacher to train a behavior-cloning policy that can either be evaluated directly or used to initialize a fine-tuned \gls{SAC} controller. Each method is evaluated for every tariff and architecture under a common protocol. Figure~\ref{fig:pipeline} illustrates the compared pipelines and their shared components.

The behavior cloning stage produces an actor that approximates the \gls{MILP} teacher on the same observation space used by the reinforcement learning environment, and is evaluated as an independent demonstration-based reference. The fine-tuning configuration initializes \gls{SAC} with the behavior-cloning weights and introduces a critic warm-up phase during which the actor remains frozen, allowing the critic to align with the behavior-cloning policy before joint updates resume. The demonstration-free variants use the same observation space, action projection, replay buffer, online update schedule, and evaluation protocol, so differences in performance can be attributed to the safety formulation, shaping, initialization, or critic design rather than to the surrounding training harness.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{5-Pipeline.pdf}
    \caption{Training pipelines compared in this work. Demonstration-free \gls{SAC} variants train directly from simulator interaction, while the demonstration-based reference uses a \gls{MILP} teacher to supervise a behavior-cloning policy that can initialize a fine-tuned \gls{SAC} controller. Potential-based shaping, \gls{CMDP} safety, and critic regularization are toggled inside the unified reinforcement-learning harness.}
    \label{fig:pipeline}
\end{figure}
...
The behavior cloning stage learns a regression of teacher actions from the observation history $\mathbf{H}_t$. It is evaluated as a stand-alone method and also serves as the initial actor for the fine-tuning reference used to measure the practical value of expert demonstrations (Section~\ref{subsec:rl}). The demonstration-free \gls{SAC} variants do not use this dataset. The observation-history length $L$ is selected by the hyperparameter search of Section~\ref{subsubsec:bc_hpo} and is propagated to the fine-tuning configuration so that the loaded actor and the reinforcement learning encoder share the same temporal context.
...
The configuration that achieves the lowest validation loss is retrained for the full epoch budget and saved as the behavior cloning artifact, together with its selected hyperparameters. This artifact serves both as a stand-alone supervised baseline and as the initialization point for the fine-tuning comparator of Section~\ref{subsec:rl}; the selected $L^{\star}$ is propagated into that configuration so that the loaded actor and the reinforcement learning encoder share the same temporal context.
...
The reinforcement learning stage trains the actor through direct interaction with the simulator. Two safety-handling formulations are considered: a penalty-based variant that keeps a single scalar reward with feasibility penalties embedded in it (Section~\ref{subsubsec:sac_penalty}), and a constrained-\gls{MDP} Lagrangian variant with separate \gls{EV} cost critics (Section~\ref{subsubsec:sac_cmdp}). The \gls{CMDP} variant is then extended with three from-scratch mechanisms: potential-based \gls{BESS} shaping, DroQ-style critic dropout, and REDQ-style randomized critic ensembles (Section~\ref{subsubsec:stabilizers}). A behavior-cloning fine-tuning run is retained as a demonstration-based comparator (Section~\ref{subsubsec:ft_protocol}). All variants share the same \gls{SAC} backbone~\cite{haarnoja2018soft}: automatic entropy tuning, $N$-step returns, interleaved online updates during rollout, and soft target updates. Unless otherwise stated, the reward critic is the standard clipped double-Q critic. All learning objectives below are written as losses to be minimized and are denoted by $\mathcal{L}$.
...
No \gls{KL} anchor is used in the unified fine-tuning runs. This warm-start quantifies the benefit and cost of expert demonstrations, allowing the proposed demonstration-free \gls{CMDP}+shaping variants to be compared against the common practice of initializing \gls{SAC} from an expert policy.

\subsubsection{Stabilization mechanisms}
\label{subsubsec:stabilizers}

Three from-scratch mechanisms are evaluated as controlled ablations on top of the \gls{CMDP} formulation. The first gives the agent a cheap physical hint through reward shaping; the other two act directly on critic overestimation, which is amplified by the long effective horizon induced by high $\gamma$.

\paragraph{Potential-based BESS shaping.}
\label{subsubsec:shaping}
Potential-based shaping~\cite{ng1999policy} modifies the training reward without changing the optimal policy: for any potential $\Phi$,

\begin{flalign}\label{eq:shaping}
    & r_t^{\text{train}} = r_t + \gamma\,\Phi(\mathbf{H}_{t+\Delta t}) - \Phi(\mathbf{H}_t),
\end{flalign}

so the cumulative shaped return differs from the raw return only by the boundary term induced by $\Phi$. The proposed potential encodes a simple economic cue for the \gls{BESS}: stored energy is more valuable when it was accumulated during relatively cheap tariff periods and can later offset more expensive grid import. Let $\sigma_t^\text{BESS}$ be the battery state of charge, $\underline{\sigma}^\text{BESS}$ its usable lower bound, $\overline{E}^\text{BESS}$ its nominal capacity, and $q_t^\zeta\in[0,1]$ the current tariff percentile over the recent tariff history. The shaping potential is

\begin{flalign}\label{eq:shaping_bess}
    & \Phi_{\mathrm{BESS}}(\mathbf{H}_t)
    =
    \omega_{\mathrm{BESS}}\,
    \overline{E}^\text{BESS}
    \max\!\left(0,\sigma_t^\text{BESS}-\underline{\sigma}^\text{BESS}\right)
    \left(1-q_t^\zeta\right),
\end{flalign}

where $\omega_{\mathrm{BESS}}$ controls the shaping strength. This term rewards moving into states with usable stored energy when the current price context is cheap, while remaining a potential-based transformation whose raw-reward optimum is preserved. Evaluation always reports the original environment reward $r_t$, so shaped and unshaped policies remain directly comparable.

\paragraph{DroQ-style critic dropout.}
The DroQ ablation inserts a small dropout probability in the reward and cost critic heads during critic training. The actor, replay buffer, safety formulation, and all other hyperparameters are kept unchanged. The purpose is not to change the objective, but to regularize the critic function class so that value estimates do not become excessively sharp around transiently favorable actions.

\paragraph{REDQ-style randomized critic ensemble.}
The REDQ ablation replaces the clipped double-Q reward critic with an ensemble of $N_Q$ critics. Each critic is trained against the same scalar target, but the Bellman target uses the minimum over a random subset of $M<N_Q$ target critics:

\begin{flalign}\label{eq:redq_target_min}
    & \widetilde{Q}_{r,\mathrm{REDQ}}^{\bar{\phi}}(\mathbf{H}'_j,a'_j)
    =
    \min_{m\in\mathcal{M}_j}
    Q_{r,m}^{\bar{\phi}_m}(\mathbf{H}'_j,a'_j),
    \qquad
    \mathcal{M}_j \subset \{1,\ldots,N_Q\},\ |\mathcal{M}_j|=M.
\end{flalign}

The same subset-min reduction is used for cost critics in the \gls{CMDP} variant. This randomized conservative target is intended to reduce overestimation while avoiding the cost of large ensembles. Together, the \gls{CMDP}, \gls{CMDP}+\(\Phi_{\mathrm{BESS}}\), \gls{CMDP}+DroQ, \gls{CMDP}+REDQ, penalty, and fine-tuned \gls{CMDP} variants define the method space evaluated in Section~\ref{sec:results}.
...
%   - Main pipeline: demonstration-free SAC variants trained per architecture,
%     tariff, and seed; BC/FT retained as demonstration-based comparator
...
%   - Columns: parameter | symbol | penalty | CMDP | CMDP+PBRS
%              | CMDP+DroQ | CMDP+REDQ | CMDP+FT | BC
...
%     cost budgets b_i (CMDP only), shaping_bess_omega, droq_dropout,
%     n_critics, m_target,
...
%     classical baselines
%   - Methods compared at evaluation: BC, SAC penalty, SAC CMDP, CMDP+PBRS,
%     CMDP+DroQ, CMDP+REDQ, CMDP+FT,
...
%     SAC CMDP, CMDP+PBRS, CMDP+DroQ, CMDP+REDQ, and CMDP+FT, benchmarked against the
...
% Core matrix: penalty | CMDP | CMDP+PBRS | CMDP+DroQ | CMDP+REDQ | CMDP+FT x 5 tariffs.
% Annual operating cost ($/year) on the held-out test year and % cost reduction
% over the RBS rule-based baseline.
% Headline: CMDP+PBRS as the demonstration-free punch, with FT as the
% demonstration-based comparator.
...
% PBRS, DroQ, and REDQ ablations. Report whether PBRS substitutes the practical
% role of BC warm-start and whether critic regularization reduces residual drift.
...
% Contrast CMDP+PBRS operation vs RBS and FT, including BESS cycling economics.
...
% - Decision guidance: when penalty vs CMDP, PBRS vs FT, and when DroQ/REDQ helps.
% - Why seed noise and critic overestimation can masquerade as structural drift.

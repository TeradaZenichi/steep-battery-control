"""Evaluate the RB controller with SafetyLayer projection by default.

Run:
    python models/RB/eval.py
"""
import json
import sys
...
from models.RB.model import load_actor
...
def main():
    ...
    out_dir = PROJECT_ROOT / "Results" / "eval" / "models" / "RB"
    ...
    for tariff in TARIFFS:
        ...
        with open(out_dir / f"rb_{tariff}.json", "w") as f:
            json.dump({"summary": {"val": val_summary, "test": test_summary},
                       "per_run": {"val": val_runs, "test": test_runs}}, f, indent=2)

    pd.DataFrame(rows).to_csv(out_dir / "rb_summary.csv", index=False)
    print(f"\nWrote {out_dir / 'rb_summary.csv'}")
